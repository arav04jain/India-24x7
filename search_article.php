...
<body style="background-color: whitesmoke;">
  ...
  <!--content-->
  <h1 style="font-size: 75px; font-weight: 750; color: red; text-align: center;">Search an Article</h1>
  <br>
  ...
  <select class="form-select" aria-label="Default select example" name="para" id="opt">
  <option value="1">Search by Date</option>
  <option value="2">Search By Writer</option>
  <option value="3">Search By Heading</option>
  </select>

  <br>
  <button type="button" class="btn btn-outline-primary" onclick="showval()">Give Value</button>
  
  <br><br>
  <p id="resultshown"></p><br>
  <button type="submit" class="btn btn-outline-primary" value='s' name='s'>Search</button>
  </div>
  <?php


  
  if(isset($_REQUEST['inputss']) && isset($_REQUEST['para'])){

  if($_REQUEST['para']=="1"){
  echo "<div class=' justify-content-center ' style='display: flex; flex-wrap: wrap;'>";
 
  $res = mysqli_query($mysqli,"SELECT COUNT(*) FROM articles where date='".date("Y-m-d", strtotime($_REQUEST['inputss']))."';");
  $num=intval(implode(mysqli_fetch_assoc($res)));
  for($x=0;$x<$num;$x++){
   
    $u="select id from articles where date='".date("Y-m-d", strtotime($_REQUEST['inputss']))."' ORDER BY id DESC limit ".$x.",1;";
...
if($_REQUEST['para']=="2"){
  ...
  $res = mysqli_query($mysqli,"SELECT COUNT(*) FROM articles where writer LIKE '%".$_REQUEST['inputss']."%';");
  $num=intval(implode(mysqli_fetch_assoc($res)));
  for($x=0;$x<$num;$x++){
   
    $u="select id from articles where writer LIKE '%".$_REQUEST['inputss']."%' ORDER BY id DESC limit ".$x.",1;";
...
if($_REQUEST['para']=="3"){
  ...
  $res = mysqli_query($mysqli,"SELECT COUNT(*) FROM articles where heading LIKE '%".$_REQUEST['inputss']."%';");
  $num=intval(implode(mysqli_fetch_assoc($res)));
  for($x=0;$x<$num;$x++){
   
    $u="select id from articles where heading LIKE '%".$_REQUEST['inputss']."%' ORDER BY id DESC limit ".$x.",1;";
